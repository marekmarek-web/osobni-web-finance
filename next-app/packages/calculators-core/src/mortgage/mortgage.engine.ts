/**
 * Mortgage calculator – state.loan = hodnota nemovitosti (u hypotéky).
 * borrowingAmount = property × LTV% nebo property − vlastní zdroje.
 */
...
export function getBorrowingAmount(state: MortgageState): number {
  if (state.product === "mortgage") {
    ...
    return Math.max(0, propertyValue - (state.own || 0));
  }
...
export function syncOwnFromLtv(state: MortgageState): number {
  const propertyValue = getPropertyValue(state);
  if (propertyValue <= 0 || state.ltvLock == null) return state.own || 0;
...
export function calculateResult(
  ...
  let finalRate = 0;
  let propertyValue = state.product === "mortgage" ? getPropertyValue(state) : state.loan + state.own;
...
/** Vlastní zdroje z LTV při známé hodnotě nemovitosti. */
export function ownFromLtvMortgage(propertyValue: number, ltv: number): number {
  const borrowing = Math.round((propertyValue * ltv) / 100);
  return Math.max(0, propertyValue - borrowing);
}
