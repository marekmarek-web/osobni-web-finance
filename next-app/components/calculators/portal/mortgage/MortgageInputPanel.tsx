...
export function MortgageInputPanel({
  ...
  const loanLabel = isMortgage ? "Hodnota nemovitosti" : isConsolidation ? "Stávající závazky" : isAuto ? "Cena vozu" : "Výše úvěru";
  const loanMinLabel = isMortgage ? "500 tis. Kč" : "20 tis. Kč";
  const loanMaxLabel = isMortgage ? "30 mil. Kč" : "2,5 mil. Kč";
  const ownResources = isMortgage ? state.own : Math.max(0, result.propertyValue - getBorrowingAmount(state));
